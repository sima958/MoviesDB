...
class MovieDetailViewController: BaseViewController {
    ...
    private let imgView: UIImageView = {
        ...
        iv.backgroundColor = ColorPicker.yellow.withAlphaComponent(0.2)
        return iv
    }()
    ...
    private let genresLabel: UILabel = {
        ...
        label.textColor = ColorPicker.black
    ...
    private lazy var favoriteBtn: UIButton = {
        ...
        btn.setTitleColor(ColorPicker.black, for: .normal)
        btn.backgroundColor = ColorPicker.yellow
        btn.titleLabel?.font = UIFont.boldSystemFont(ofSize: 20)
        return btn
    }()
    
    init(movie: Movie, movieApi: AbstractNetworkService) {
        self.movie = movie
        defer {
            ...
            if let isFavorite = movie.isFavorite, isFavorite {
                favoriteBtn.setTitleColor(ColorPicker.black, for: .normal)
                favoriteBtn.backgroundColor = UIColor.lightGray
                favoriteBtn.setTitle("Remove from favorites", for: .normal)
            } else {
                favoriteBtn.setTitleColor(ColorPicker.black, for: .normal)
                favoriteBtn.backgroundColor = ColorPicker.yellow
